...
def test_build_record_diagram_contains_chain_ids(tmp_registry):
    mermaid, meta = build_record_diagram(str(tmp_registry), "C4-0001", max_nodes=200)
    assert isinstance(mermaid, str) and mermaid.strip()
    # Must include the chain IDs
    assert "C4-0001" in mermaid
    assert "C3-0001" in mermaid
    assert "C2-0001" in mermaid
    assert "C1-0001" in mermaid
